  ...
  function nomoji(txt, prefix) {
    ...
    try {
      for (_iterator.s(); !(_step = _iterator.n()).done; ) {
        ...
        chars.push([_char3, _char3.codePointAt(0).toString(16)]);
      }
    } catch (err) {
      _iterator.e(err);
    } finally {
    ...
    for (var i = 0; i < chars.length; i++) {
      ...
      while (emoji[chars[i][1]]) {
        var _chars$i2 = _slicedToArray(chars[i++], 2),
          _char2 = _chars$i2[0],
          _codePoint = _chars$i2[1];
        result.push(_codePoint);
        emoji = emoji[_codePoint];
      }
      if (result.length) {
        results += '<img draggable="false" class="emoji" src="'
          .concat(prefix, "svg/")
          .concat(result.join("_").toLowerCase(), '.svg">');
        i--;
      } else {
        results += _char;
      }
